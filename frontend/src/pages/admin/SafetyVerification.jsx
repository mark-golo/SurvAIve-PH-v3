...
import { CheckCircle, AlertTriangle, Download, Users } from 'lucide-react'
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setReported(sos.map(s => ({
        ...
        accounted: s.rescue_status === 'rescued',
      })))
      setNoReport(victims
        .filter(v => !sosContactSet.has(v.contact_number))
        .map(v => ({
          id:            v.id,
          name:          v.name,
          barangay:      v.barangay ?? '—',
    ...
    <AdminLayout title="Safety Verification">
      <div className="p-4 space-y-4">
        ...
        <GlassCard glow>
          ...
          {total === 0 ? (
            <p className="text-xs text-slate-500">No registered constituents or SOS reports yet.</p>
          ) : (
            <>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div>
            ...
            {reported.length === 0
              ? <p className="text-xs text-slate-500 py-4 text-center">No SOS reports</p>
              : (
          ...
          <div>
            ...
            {noReport.length === 0
              ? <p className="text-xs text-slate-500 py-4 text-center">All registered constituents have reported</p>
              : (
                <div className="space-y-2">
                  {noReport.map(r => (
                    <div key={r.id} className={`glass rounded-xl p-3 border transition-all ${
