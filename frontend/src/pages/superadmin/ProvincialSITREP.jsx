...
// ── Print ──────────────────────────────────────────────────────────────────────
function doPrint({ summary, muniRows, prov, generatedAt, fromDate, toDate }) {
  const win = window.open('', '_blank')
  const dateRange = fromDate === toDate ? fromDate : `${fromDate} to ${toDate}`
  const tableRows = muniRows.map(r => `
    <tr>
  ...
  win.document.write(`<!DOCTYPE html><html><head>
    <title>Provincial SITREP — ${prov}</title>
    <style>
    ...
    <div class="header">
      <h1>Provincial Disaster Situation Report (SITREP)</h1>
      <div class="meta">Province of ${prov} &nbsp;|&nbsp; Period: ${dateRange}</div>
      <div class="meta">Generated: ${generatedAt?.toLocaleString('en-PH', { dateStyle: 'full', timeStyle: 'short' }) ?? '—'}</div>
      <div class="meta" style="margin-top:8px;font-weight:bold;font-size:10px;color:#888">SurvAIve PH — DRRM Provincial Command System</div>
    </div>
    ...
    <div class="section">
      <h2>II. Municipality Breakdown</h2>
      <table>
        <tr><th>Municipality</th><th>Registered</th><th>Safe</th><th>Not Safe</th><th>No Response</th><th>Critical</th><th>Status</th></tr>
        ${tableRows || '<tr><td colspan="7">No data in selected period</td></tr>'}
      </table>
    </div>

    <div class="section">
      <h2>III. Most Affected Municipalities</h2>
      <table>
        <tr><th>#</th><th>Municipality</th><th>Total SOS</th><th>Critical</th><th>Not Safe</th></tr>
        ${muniRows.slice(0, 5).map((r, i) =>
...
// ── Main component ─────────────────────────────────────────────────────────────
export function ProvincialSITREP() {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      const [sosRes, vicRes] = await Promise.all([
        supabase
          .from('sos_reports')
          .select('id, municipality, status, rescue_status, ai_priority_score, people_count, special_conditions, created_at, notes')
          .eq('province', prov)
      ...
      ;(vicRes.data ?? []).forEach(v => {
        const m = v.municipality ?? 'Unknown'
        victimsByMuni[m] = (victimsByMuni[m] ?? 0) + 1
      })
      setData({ sosList, victimsByMuni })
      setGeneratedAt(new Date())
    } catch (e) {
      console.error('Provincial SITREP generate error', e)
    } finally {
      setLoading(false)
    }
  ...
  const muniRows = useMemo(() => {
    if (!data) return []
    const { sosList, victimsByMuni } = data
    const map = {}
    sosList.forEach(r => {
      const m = r.municipality ?? 'Unknown'
      if (!map[m]) map[m] = { safe: 0, notSafe: 0, noResp: 0, critical: 0, total: 0 }
      map[m].total++
      if (isSafe(r))       map[m].safe++
      if (isNotSafe(r))    map[m].notSafe++
      if (isNoResponse(r)) map[m].noResp++
      if (isCritical(r))   map[m].critical++
    })
    return Object.entries(map)
      .map(([b, v]) => ({ b, registered: victimsByMuni[b] ?? 0, ...v }))
      ...
      <div className="flex flex-col h-[calc(100vh-56px)] overflow-y-auto">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-sm font-bold text-white uppercase tracking-wide leading-tight">
                  Provincial Disaster Situation Report
                </h1>
                <p className="text-[11px] text-slate-500 mt-0.5">
                  {prov ? `Province of ${prov}` : 'All Provinces'}
                  {generatedAt
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {preset === 'custom' && (
                <div className="flex items-center gap-2">
                  <input type="date" value={fromDate} onChange={e=>setFromDate(e.target.value)}
                    className="bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.1)] rounded-lg px-2 py-1.5 text-[11px] text-white outline-none focus:border-[rgba(139,92,246,0.4)]" />
              ...
              <div className="flex gap-2 ml-auto">
                <button onClick={generate} disabled={loading}
                  className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold text-white transition-all bg-gradient-to-r from-[#7c3aed] to-[#8b5cf6] disabled:opacity-50 hover:opacity-90">
                  <RefreshCw size={13} className={loading ? 'animate-spin' : ''} />
                  {loading ? 'Generating…' : 'Generate Report'}
                </button>
                {hasData && (
                  <button onClick={() => doPrint({ summary, muniRows, prov, generatedAt, fromDate, toDate })}
                    className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-[#8b5cf6] border border-[rgba(139,92,246,0.3)] hover:bg-[rgba(139,92,246,0.08)] transition-all">
        ...
        {!hasData && !loading && (
          <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center px-6 py-16">
            <div className="w-16 h-16 rounded-2xl bg-[rgba(139,92,246,0.08)] border border-[rgba(139,92,246,0.15)] flex items-center justify-center">
              <ClipboardList size={28} className="text-[rgba(139,92,246,0.5)]" />
            </div>
            <div>
              ...
              <p className="text-slate-500 text-sm mt-1 max-w-xs leading-relaxed">
                Select a date range and click <strong className="text-slate-300">Generate Report</strong> to produce the Provincial SITREP for {prov ?? 'your province'}.
              </p>
            </div>
          </div>
        )}
        ...
        {loading && (
          <div className="flex-1 flex items-center justify-center">
            <p className="text-slate-400 text-sm animate-pulse">Generating Provincial SITREP…</p>
          </div>
        )}
        ...
        {hasData && !loading && (
          <div className="p-4 space-y-6">
            ...
            {/* ── II. Municipality breakdown table ─────────────────────────── */}
            <section>
              <SectionTitle>II. Municipality Breakdown</SectionTitle>
              <div className="glass rounded-2xl p-4">
                <Tbl
                  heads={['Municipality', 'Registered', 'Safe', 'Not Safe', 'No Response', 'Critical', 'Status']}
                  rows={muniRows.map(r => {
            ...
            {muniRows.length > 0 && (
              <section>
                <SectionTitle>III. Municipality Severity Heatmap</SectionTitle>
                      ...
                      >
                        ...
                        <div className="mt-2 h-1 rounded-full bg-white/10 overflow-hidden">
                          <div
                            ...
                            style={{
                              width:      `${Math.min(100, (row.critical / Math.max(1, muniRows[0]?.critical || row.total)) * 100)}%`,
                              background: color,
                            }}
                          />
                        </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="glass rounded-2xl p-4">
                  <p className="text-xs font-bold text-slate-300 mb-3">Most Affected Municipalities</p>
                  {muniRows.length === 0
                    ? <p className="text-xs text-slate-500">No data</p>
                    : muniRows.slice(0, 5).map((r, i) => {
                        const pct = Math.round((r.total / Math.max(1, muniRows[0].total)) * 100)
                        return (
                        ...
                        {criticalIncidents.map(r => (
                          <div key={r.id} className="bg-[rgba(239,68,68,0.06)] border border-[rgba(239,68,68,0.15)] rounded-lg px-3 py-2">
                            <div className="flex items-center justify-between gap-2">
                              <p className="text-xs font-semibold text-white truncate">{r.municipality ?? 'Unknown'}</p>
            ...
            <div className="text-center py-4">
              <p className="text-[10px] text-slate-600">
                SurvAIve PH — DRRM Provincial Command System · Report generated {generatedAt?.toLocaleString('en-PH', { dateStyle: 'long', timeStyle: 'short' })}
              </p>
            </div>
